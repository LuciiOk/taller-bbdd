...
-- carga de datos --
copy temporal from 'C:\Users\lport\Desktop\scripts\taller 2\inmuebles.csv'
...
-- insertar supervisores en los repectivos vendedores --
update vendedores as vendU 
	set id_supervisor = vend.id_vendedor 
	from temporal as tempo
	join vendedores as vend on vend.nombre = tempo.supervisor
	where vendU.nombre = tempo.vendedor;
...
insert into operaciones (fechaalta, fechaoperacion, precio, id_propiedad, tipo_operacion, vendedor, comprador)
	select cast(fechaalta as DATE), cast(fechaventa as DATE), precioventa, prop.id_propiedad, oper.id_tipooperacion, vend.id_vendedor, rutcomprador from temporal as tempo
	...
	left join vendedores as vend on tempo.vendedor = nombre;
