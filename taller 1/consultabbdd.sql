...
--select v.nombre, vend.nombre from vendedores as v
--left join vendedores as vend on v.id_supervisor = vend.id_vendedor order by v.nombre

--select distinct  vendedor, supervisor from temporal where vendedor is not null order by vendedor
...
copy (select fechaalta, tipopropiedad, tipooperacion, provincia, superficie, construidos, precioventa, fechaventa, vendedor, supervisor, rutdueno, dueno, celular, email, comprador, rutcomprador
from temporal) 
to 'C:\Users\lport\Desktop\hola2.csv' (format CSV)
